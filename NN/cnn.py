...
from util import LoadData, Load, Save, DisplayPlot, SavePlot
...
def Conv2DBackward(grad_y, x, y, w):
    ...
    grad_w = np.transpose(Conv2D(np.transpose(x,[3,1,2,0]),np.transpose(grad_y,[1,2,0,3]),(I-1,J-1)),[1,2,0,3])
...
def CNNUpdate(model, eps, momentum):
    ...
    model['vW1'] = momentum*model['vW1'] + eps*model['dE_dW1'] if 'vW1' in model else (eps*model['dE_dW1']);
    model['vW2'] = momentum*model['vW2'] + eps*model['dE_dW2'] if 'vW2' in model else (eps*model['dE_dW2']);
    model['vW3'] = momentum*model['vW3'] + eps*model['dE_dW3'] if 'vW3' in model else (eps*model['dE_dW3']);
    model['vb1'] = momentum*model['vb1'] + eps*model['dE_db1'] if 'vb1' in model else (eps*model['dE_db1']);
    model['vb2'] = momentum*model['vb2'] + eps*model['dE_db2'] if 'vb2' in model else (eps*model['dE_db2']);
    model['vb3'] = momentum*model['vb3'] + eps*model['dE_db3'] if 'vb3' in model else (eps*model['dE_db3']);
...
def main():
    ...
    model, stats = Train(model, CNNForward, CNNBackward, CNNUpdate, eps[0], momentum[0], num_epochs, batch_size[0])
    ...
    for e in eps:
        ...
        model, stats = Train(model, CNNForward, CNNBackward, CNNUpdate, e, 0.0, num_epochs, 100)
    ...
    for m in momentum:
        ...
        model, stats = Train(model, CNNForward, CNNBackward, CNNUpdate, 0.1, m, num_epochs, 100)
    ...
    for b in batch_size:
        ...
        model, stats = Train(model, CNNForward, CNNBackward, CNNUpdate, 0.1, 0.0, num_epochs, b)
