...
def ReLUBackward(grad_y, x, y):
    ...
    Returns:
        grad_x: Gradients wrt. the inputs.
    """
    grad_x = np.multiply(grad_y,(x > 0).astype(int));
    return grad_x
    ...
    for epoch in range(num_epochs):
        ...
        for step in range(num_steps):
            ...
            print(('Epoch {:3d} Step {:2d} Train CE {:.5f} '
                   ...
                   'm {:5f} '
                   'b {:5f}').format(
                epoch, step, train_ce, train_acc,eps,momentum,batch_size))
        ...
        print(('Epoch {:3d} '
               ...
               'm {:5f} '
               'b {:5f}\n').format(
            epoch, valid_ce, valid_acc,eps,momentum,batch_size))
...
def Evaluate(inputs, target, model, forward, batch_size=-1):
    ...
    for step in range(num_steps):
        ...
        #print(np.max(prediction))
            ...
            #PoorFaces = np.concatenate((PoorFaces,np.reshape(x,[48,48])),axis=0)
    
    #PoorFaces = PoorFaces[48:,:]
    #PoorFaces = np.concatenate((PoorFaces[:2*48,:],PoorFaces[2*48:,:]),axis=1)
    #plt.matshow(PoorFaces,cmap=plt.cm.gray)
    #input("press any key")
...
def main():
    ...
    model = Load("C:\\Users\\Adam\\Documents\\Masters\\Machine Learning\\A2\\NN\\NN_Results\\0.01\\0.0\\100\\nn_model.npz")
